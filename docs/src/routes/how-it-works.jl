    ...
    Div(:class => "space-y-14 max-w-3xl mx-auto",
        ...
        section("Direct opening and static hosting",
            ...
            para("For the simplest local handoff, pass ", code("single_file=true"),
                ". Snapshot embeds its generated runtime assets into the document, so the resulting HTML can be opened directly without running a local server. This is convenient for downloads and platforms that accept active HTML, but base64 increases binary size, the browser cannot cache individual modules, and a host may still block scripts or WebAssembly. Use the split format for a deployed site."),
            para("For a Therapy site, use a small Snapshot build environment to export into the site's public directory, then a separate Therapy environment to mount it with ",
                code("staticfiles(app, \"public/notebook\", \"notebook\")"),
                ". The separate environments are intentional because Snapshot's Pluto stack and Therapy currently resolve different HTTP major versions. ",
                code("julia --project=therapy-site therapy-site/app.jl dev"), " serves it locally; ",
                code("julia --project=therapy-site therapy-site/app.jl build"),
                " copies the complete mount into the site's static output and writes ",
                code(".nojekyll"), " for GitHub Pages. Set Therapy's ",
                code("base_path=\"/REPOSITORY\""),
                " for a project Pages URL. Snapshot is needed at build time only; the deployed site has no Julia server. Mount only trusted notebook HTML on an application's origin; publish untrusted exports on a separate origin.")),
