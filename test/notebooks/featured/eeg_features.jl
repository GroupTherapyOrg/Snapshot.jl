...
#> [frontmatter]
#> order = "13"
#> title = "EEG band-power features"
#> tags = ["signal", "eeg", "beacon"]
#> description = "Interactive EEG spectral band-power feature extraction — pure Julia, runs in the browser via WebAssembly (no server)."
...
md"""# EEG spectral band-power features

A self-contained EEG feature extractor: a synthetic signal (sum of physiological
rhythms) is analysed with a **pure-Julia DFT** and reduced to the canonical clinical
band powers. Everything below the slider runs **in your browser as WebAssembly** — no
Julia server, no install. Drag the slider to change the **alpha rhythm amplitude** and
watch the band powers update live.
"""

# ╔═╡ aaaa0003-0000-0000-0000-000000000003
md"**Alpha (8–13 Hz) amplitude:**"
...
function eeg_band_powers(alpha_amp::Float64)
    ...
    for n in 1:N
        t = (n - 1) / fs
        x[n] = 0.5*sin(2π*2*t) + 0.4*sin(2π*6*t) + alpha_amp*sin(2π*10*t) +
               0.3*sin(2π*20*t) + 0.2*sin(2π*40*t)
    end
    half = N ÷ 2
...
md"""**Relative alpha power:** $(rel)

(alpha-band fraction of total spectral power — rises as you increase the alpha amplitude)
"""
