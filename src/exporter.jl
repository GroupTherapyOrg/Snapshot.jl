    ...
    if therapy
        ...
        if fragment
            ...
            write(frag_path, generate_therapy_html(notebook, output_dir, name, islands_dirname;
                theme_picker=false, fragment=true, fragment_dependencies, assets_base))
        end
...
function generate_therapy_html(notebook, output_dir::AbstractString, name::AbstractString,
                               ...
                               fragment_dependencies::Symbol=:inline,
                               assets_base::AbstractString="")
    fragment_dependencies in (:inline, :host) || throw(ArgumentError(
        "fragment_dependencies must be :inline or :host, got $(repr(fragment_dependencies))"))
    ...
    function _error_html(body)
        ...
        msg = body isa AbstractDict ?
              something(getkey(body, :msg), getkey(body, :plain_error), "Notebook cell failed") :
              string(body)
        ...
        # Match Pluto's native concise/no-stack error component structure. The
        # exported document already carries Pluto's own `jlerror` CSS, remapped
        # onto Snapshot/DaisyUI theme tokens in pluto_css.jl. Keeping the native
        # DOM shape gives every collection theme Pluto's established hierarchy
        # without shipping the editor runtime or exposing a machine-local trace.
        return string(
            "<jlerror class=\"snapshot-error\">",
            "<div class=\"error-header\"><secret-h1>Notebook cell failed</secret-h1></div>",
            "<header translate=\"no\"><p>",
            _esc(first(summary, 2000)),
            "</p></header></jlerror>",
        )
    end
    has_toc = false
    ...
    for id in notebook.cell_order
        ...
        # PlutoUI.TableOfContents needs Pluto's own DOM — skip its captured widget
        # output and render a lean aside ToC in the shell instead. Detect the
        # semantic output element, never a source-code substring: comments,
        # strings, aliases, and helper functions may legitimately mention
        # `TableOfContents` without being the widget.
        is_toc_output = mime == "text/html" &&
            occursin(r"""<(?:nav|div)\b[^>]*\bclass\s*=\s*["'][^"']*\bplutoui-toc\b""", bodystr)
        if is_toc_output
            has_toc = true
            continue
        end
...
"""
    ...
    # dashboard + collection shells (data-theme + pinned DaisyUI themes + per-theme
...
/* Lezer-Julia token highlight. These consume the same shared syntax tokens as
   Pluto/CodeMirror in PLUTO_VAR_MAP, so upstream and exported code cannot drift. */
.tok-keyword,.tok-controlKeyword,.tok-definitionKeyword,.tok-moduleKeyword,.tok-operatorKeyword{color:var(--snapshot-syntax-keyword)}
.tok-comment,.tok-lineComment,.tok-blockComment{color:var(--pl-muted);font-style:italic}
.tok-string,.tok-character,.tok-special{color:var(--snapshot-syntax-string)}
.tok-number,.tok-integer,.tok-float,.tok-bool,.tok-atom,.tok-constant{color:var(--snapshot-syntax-number)}
.tok-typeName{color:var(--snapshot-syntax-type)}
.tok-macroName{color:var(--snapshot-syntax-macro)}
.tok-function,.tok-definition{color:var(--snapshot-syntax-function)}
...
.plutoui-toc.toc-embedded.aside{
  z-index:var(--snapshot-toc-z,30);
  top:var(--snapshot-toc-top,4.5rem);
  right:var(--snapshot-toc-right,0);
  max-height:var(--snapshot-toc-max-height,calc(100vh - 6rem));
  box-shadow:0 2px 14px color-mix(in oklab,var(--color-base-content) 10%,transparent)
}
...
""" : ""
    if fragment
        ...
        # assets_base makes the shim src absolute & base-aware (a placeholder the
        # host rewrites per base_path).
        scoped = string(dui_css, "\n", pluto_css)
        # Rewrite document-root selectors into the component scope. Do the
        # combined selector first, then individual selector tokens. These patterns
        # are intentionally selector-shaped (look ahead to `{`/attributes), so
        # prose, URLs, and property values are untouched.
        scoped = replace(scoped, r"\bhtml\s*,\s*body\b" => ":scope")
        scoped = replace(scoped, r"(?<![-\w])(?:html|body)(?=\s*\{)" => ":scope")
        scoped = replace(scoped, r":root(?=\s*(?:\[|\{|:|\.))" => ":scope")
        ...
        # Fail loudly if future upstream formatting defeats the scoper. Theme
        # selectors are emitted separately above; the component block itself must
        # never retain a document-root selector.
        ...
        return string(
            "<div class=\"snap-notebook\" data-snapshot-requires=\"daisyui-themes@5.6.2 katex@0.11.1\">\n",
            fragment_resources,
            "<style>\n", frag_theme_css, "\n", frag_css, "\n</style>\n",
            toc_html(true), "\n",
